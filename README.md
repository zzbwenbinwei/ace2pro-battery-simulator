...
| **强制解容电量** | 300mAh | 最小解容电量 | 200-500mAh |
...
# 运行时配置
/data/local/tmp/battery_config.sh
```
...
### Q: WebUI 无法访问？
**A:** 检查以下项目：
1. 模块是否正常启动：`ps | grep battery`
2. 端口是否被占用：`netstat -tulpn | grep 8080`
3. 防火墙是否阻止：检查设备防火墙设置

### Q: 电量卡在100%不下降？
**A:** 尝试以下解决步骤：
1. 重启电池服务：`pkill battery_simulator`
2. 删除缓存文件：`rm -f /data/local/tmp/dian.log`
3. 重启设备
...
2. 删除残留文件：
   ```bash
   rm -rf /data/adb/modules/ace2pro-battery-simulator
   rm -f /data/local/tmp/dian.log
   rm -f /data/local/tmp/max_dian.log
   rm -f /data/local/tmp/battery_service.log
   ```
3. 重启设备
...
### 开发构建
```bash
git clone https://github.com/your-repo/battery-mod.git
cd battery-mod
./build.sh
```
